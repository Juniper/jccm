...
export const Login = ({ isOpen, onClose }) => {
    ...
    const requestUserLookup = async (cloud, email) => {
        try {
            const data = await electronAPI.saLookupApiEndpoint({
                cloud,
                email,
            });
            ...
            showMessageBar({
                message: `Error lookup the cloud api server endpoint - ${err}`,
                intent: 'error',
            });
    ...
    const requestUserLogin = async (
        ...
        try {
            const data = await electronAPI.saLoginUser({
                cloud,
                region,
                email,
                password,
    ...
    const onLoginButton = async (cloud, email, password) => {
        ...
        if (lookupResult?.status === 'success') {
            console.log('User found:', lookupResult.regions);
    ...
    const requestGoogleSSOUserLogin = async (authCode) => {
        try {
            ...
            return {
                status: 'error',
                message: 'Google SSO Login failed!',
                error: error,
            };
        }
    };
    ...
    const startGoogleSSOAuth = async (cloud, region) => {
        ...
        try {
            const data = await electronAPI.saGetGoogleSSOAuthCode({
                cloud,
                region,
            });
            ...
            if (data.login) {
                ...
                return {
                    status: 'success',
                    message: 'SSO Login ask successful!',
                };
            } else {
                console.log('SSO Login ask failed!');
                return {
                    status: 'error',
                    message: 'SSO Login ask failed!',
                    error: data.error,
                };
            }
        } catch (error) {
        ...
        >
            ...
            >
                ...
                >
                    <Popover
                        positioning={{
                            positioningRef: regionSelectPositioningRef,
                        }}
                        trapFocus
                    ...
                    <Popover
                        positioning={{
                            positioningRef: passcodeInputPositioningRef,
                        }}
                        trapFocus
