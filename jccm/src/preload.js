...
contextBridge.exposeInMainWorld('electronAPI', {
    ...
    saLookupApiEndpoint: (args) =>
        ipcRenderer.invoke('saLookupApiEndpoint', args),
    ...
    saGetGoogleSSOAuthCode: (args) =>
        ipcRenderer.invoke('saGetGoogleSSOAuthCode', args),
    saGoogleSSOAuthCodeReceived: (callback) =>
        ipcRenderer.on('saGoogleSSOAuthCodeReceived', (event, code) =>
            callback(code)
        ),
    saLoginUserGoogleSSO: (args) =>
        ipcRenderer.invoke('saLoginUserGoogleSSO', args),

    saWhoamiUser: () => ipcRenderer.invoke('saWhoamiUser'),
    saSetThemeUser: (args) => ipcRenderer.invoke('saSetThemeUser', args),
    saGetCloudInventory: (args) =>
        ipcRenderer.invoke('saGetCloudInventory', args),
    ...
    saSetLocalInventory: (args) =>
        ipcRenderer.invoke('saSetLocalInventory', args),
    ...
    startSSHConnection: (config) =>
        ipcRenderer.send('startSSHConnection', config),
    sendSSHInput: (id, data) => ipcRenderer.send(`sendSSHInput-${id}`, data),
    resizeSSHSession: (id, size) =>
        ipcRenderer.send(`resizeSSHSession-${id}`, size),
    disconnectSSHSession: (id) =>
        ipcRenderer.send('disconnectSSHSession', { id }),
    sshSessionOpened: (callback) =>
        ipcRenderer.on('sshSessionOpened', (event, id) => callback(id)),
    onSSHDataReceived: (callback) =>
        ipcRenderer.on('sshDataReceived', (event, data) => callback(data)),
    onSSHErrorOccurred: (callback) =>
        ipcRenderer.on('sshErrorOccurred', (event, error) => callback(error)),
    onSSHSessionClosed: (callback) =>
        ipcRenderer.on('sshSessionClosed', (event, data) => callback(data)),
    ...
    saExecuteJunosCommand: (args) =>
        ipcRenderer.invoke('saExecuteJunosCommand', args),
    saGetDeviceFacts: (args) => ipcRenderer.invoke('saGetDeviceFacts', args),
    saCommitJunosSetConfig: (args) =>
        ipcRenderer.invoke('saCommitJunosSetConfig', args),
